...
Goal 1 is done. Goals 2–10 are planned in detail but not built.

Roughly two hours have gone on the planning above; the auth slice before it was the bulk of the
work so far.
...
| # | Focus | Est. |
|---|---|---|
| 1 | Billing models, migrations, RLS policies, immutability triggers | 2:15 |
| 2 | Subscriptions and collaborators (goals 2, 5) | 2:15 |
| 3 | Invoice lifecycle and audit timeline (goals 3, 4, 9) | 2:00 |
| 4 | Invoice search and bulk generation (goals 6, 7) | 2:00 |
| 5 | Dashboard, alerts, seed data, **first deploy** (goals 8, 10) | 2:30 |
| 6 | Frontend core — login, subscriptions, invoices | 2:30 |
| 7 | Frontend remainder and documentation | 2:30 |

Three ordering decisions worth recording:

- **RLS goes in session 1, not last.** It constrains every query in the system. Retrofitting it onto
  a finished app means rewriting the tests and discovering that management commands were broken all
  along.
- **Deploy in session 5, with the app unfinished.** Free-tier hosting fails in ways you cannot
  predict, and the database has to permit `FORCE ROW LEVEL SECURITY` or the entire second security
  layer is decorative. Finding that out on the last evening would be unrecoverable; finding it out
  in session 5 costs an hour and a different provider.
- **Frontend last**, because it cannot be built against an API that does not exist yet.

### Estimate versus reality, so far

The seven sessions total sixteen hours against the brief's twelve-hour guide. I am recording that
now rather than discovering it at hour eleven. The cut list, decided in advance and in order:
concurrency tests, the timestamp drift-checker, the revenue chart (falls back to a table), inline
subscription editing. What does not get cut under any circumstances: the invoice state machine and
its rejection messages, the append-only timeline, server-side filtering, and the RLS test suite.
